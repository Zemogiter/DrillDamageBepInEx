...
namespace DrillDamage
{
    ...
    internal class Plugin : BaseUnityPlugin
    {
        ...
        {
            MethodInfo method = typeof(Drillable_Transpiler_Patch).GetMethod("Transpiler");
            MethodInfo method2 = typeof(SeamothDrillable_Patch).GetMethod("Patch");
            Patch_OnDrill(harmony, typeof(Drillable), method);
            if (ConfigAffectSeamothArms.Value == true)
            {
                Patch_OnDrill(harmony, GetType(Path.Combine(Paths.PluginPath, "ModdedArmsHelper/ModdedArmsHelper.dll"), "ModdedArmsHelper.API.SeamothDrillable"), method2);
            }
        }
